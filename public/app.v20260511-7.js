...
function renderHome() {
  ...
  const heroItems = (carousel.length ? carousel : fallbackCarousel).slice(0, 5);
  const hero = heroItems[0] || published[0] || state.dramas[0];
    ...
    <section class="hero">
      ...
      <div class="hero-marquee">
        <div class="hero-track">
          ${[...heroItems, ...heroItems]
            .map((item) => `<button class="hero-chip" data-open-drama="${item.id}"><img src="${item.cover}" alt=""><span>${item.title}</span></button>`)
            .join("")}
        </div>
      </div>
